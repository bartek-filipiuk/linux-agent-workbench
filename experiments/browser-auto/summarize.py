...
lines=['| Zadanie | Auto | First | Browser Use | Ultrafast | Zmiana mediany Auto vs First |','|---|---:|---:|---:|---:|---:|']
for task in tasks:
...
lines+=['','Średnie składników ze wszystkich prób danej grupy (nie tylko sukcesów):','','| Zadanie / silnik | n | Setup s | Zadanie s | Model s | Jev s | Pozostałe s | Wywołania model / Jev | Koszt USD |','|---|---:|---:|---:|---:|---:|---:|---:|---:|']
for task in tasks:
...
lines+=['','Wszystkie serie (routing i desktop mają osobne kryteria):','','| Plik | Rodzaj | Wynik | Raportowany USD |','|---|---|---:|---:|']
for name,d in docs.items():
 if 'results' not in d and 'checks' not in d:continue
 rs=d.get('results',d.get('checks',[]));kind='desktop' if 'checks' in d else 'routing' if name.startswith('decisions-') else 'wykonanie'
 passed=sum(bool(r.get('passed',r.get('success',False))) for r in rs)
 lines+=['| '+ ' | '.join([name,kind,f'{passed}/{len(rs)}',f'{sum(r.get("costUsd",0) for r in rs):.6f}' if kind!='desktop' else 'nie zmierzono tutaj'])+' |']
lines+=['','Każda próba wykonawcza, również nieudana (czas zadania w s):','','| Seria | Zadanie | Silnik | Powt. | Wynik | Czas s | Model / Jev | USD |','|---|---|---|---:|---|---:|---:|---:|']
for name,d in docs.items():
 if name.startswith('decisions-') or 'checks' in d:continue
 for r in d.get('results',[]):
  lines+=['| '+' | '.join([name,r['task'],r['engine'],str(r['repeat']),'PASS' if r.get('success') else 'STOP (celowy)' if name.startswith('stop-') else 'FAIL',fmt(r.get('taskMs')),f'{r.get("primaryCalls","—")} / {r.get("jevCalls","—")}',f'{r.get("costUsd",0):.6f}'])+' |']
output.write_text('\n'.join(lines)+'\n')
