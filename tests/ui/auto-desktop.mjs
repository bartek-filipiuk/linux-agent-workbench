...
if (process.env.LAW_INSTANCE !== "jev-auto") throw new Error("Run with LAW_INSTANCE=jev-auto to use the isolated experiment");
const root = fileURLToPath(new URL("../../", import.meta.url));
const artifacts = fs.mkdtempSync(path.join(os.tmpdir(), "law-jev-auto-desktop-"));
...
try {
  ...
  await newTask("jev-auto", "Open https://example.com, open its More information link, then report the resulting page title and URL. Use only the browser.", testModel);
  ...
  const passed = run.state === "completed" && run.browserEngine === "jev-auto" && run.jev?.decisions > 0 && new URL(browser.url).hostname === "www.iana.org" && new URL(browser.url).pathname === "/help/example-domains" && browser.title === "Example Domains";
  report.checks.push({ check: "auto_navigation", passed, elapsedMs: Date.now() - started, state: run.state, engine: run.browserEngine, jev: run.jev, browser: { url: browser.url, title: browser.title }, final: run.finalText });
  if (!passed) throw new Error("Jev Auto live navigation failed independent URL check");
  ...
  await newTask("jev-auto", "Open https://example.com, then follow its More information link. Find five technical standards, visit each standard's page and report its title. Use only the browser.");
