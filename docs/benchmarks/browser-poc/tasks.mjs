...
function wizard(count) {
  const names = cities.slice(0,count);
  return {
    id: `wizard-${count}`, category: 'long-ui',
    goal: `Prepare all ${count} trip segments in this order: ${names.join(', ')}. For EVERY segment set Destination to the corresponding city, Traveler to Ada, and Transport to Train. Use Next segment until the final Review. Stop at the completed review. Do not skip any segment.`,
    ...
    async verify(browser) {
      ...
      const checks = {count:records?.length===count, values:names.every((city,i)=>records?.[i]?.city===city&&records[i].person==='Ada'&&records[i].mode==='Train')};
