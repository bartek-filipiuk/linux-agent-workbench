...
import {Store,RunController,OpenRouterAdapter} from '/home/bartek/linux-agent-jev/services/agentd/dist/index.js';
import {browserExecutor} from '/home/bartek/linux-agent-jev/services/agentd/dist/tools/browser-tools.js';
...
const engines=flag('--engines','app-first,ultrafast,browser-use').split(',');
if(engines.some(e=>!['app-first','ultrafast','browser-use'].includes(e)))throw Error('Unknown engine');
const selected=flag('--tasks','local').split(',');
const suite=tasks.filter(t=>selected.includes(t.id)||(selected.includes('local')&&t.category!=='live'));
...
const report={createdAt:new Date().toISOString(),model,effort:'low',upstream:'google-ai-studio',jev:'jev-1.13.0',engines,repeats,
  ...
  settings:{timeoutMs,viewport:{width:1120,height:780},channel:'chromium',warmAfter:flag('--warm-after',null)},results:[]};
report.source.hashes=Object.fromEntries(['run.mjs','driver.py','tasks.mjs'].map(file=>[file,createHash('sha256').update(fs.readFileSync(path.join(root,file))).digest('hex')]));
...
async function appDriver(browser,task,goal,workDir){
  ...
  const config=JevConfig.parse({apiKey:jevKey,model:'jev-1.13.0'});
  const rc=new RunController({store,provider:'openrouter',adapter:new OpenRouterAdapter({model,effort:'low',provider:'google-ai-studio',apiKey:openrouterKey}),
    ...
    hybrid:{evaluator:new JevClient(config),observation:()=>lastObservation,minConfidence:config.minConfidence,strategy:'first'}},
...
async function pythonDriver(browser,task,goal,workDir,engine,profile){
  ...
  const child=spawn(path.join(root,'.venv/bin/python'),[path.join(root,'driver.py')],{cwd:workDir,env:childEnv,stdio:['pipe','pipe','pipe','pipe']});
  activeChild=child;
  ...
  return {go:async()=>{
    ...
    if(daemonRuntime&&fs.existsSync(daemonRuntime)){
      ...
      try{execFileSync(path.join(root,'.venv/bin/python'),['-c',"from browser_harness.admin import restart_daemon; restart_daemon('poc')"],
...
try{
  outer:for(const [taskIndex,task] of suite.entries())for(let repeat=1;repeat<=repeats;repeat++)for(let i=0;i<engines.length;i++){
    ...
    try{
      ...
      driver=engine==='app-first'?await appDriver(browser,task,goal,workDir):await pythonDriver(browser,task,goal,workDir,engine,profile);
