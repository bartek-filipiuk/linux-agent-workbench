...
const instance=process.env.LAW_CAPTURE_INSTANCE ?? 'tutorial-final';
...
let recorder;
const report={captureDate:new Date().toISOString(),prompt:'Open https://github.com/microsoft/playwright/releases. Find the latest stable release, report its version, release date and source link. Leave its release page visible. Use only the browser.',events:[],markers:[]};
...
try{
 ...
 await wait(()=>Promise.resolve(fs.existsSync(dir+'/capture.trigger')),600000);
 fs.unlinkSync(dir+'/capture.trigger');
 await page.exposeFunction('recordEvent',e=>report.events.push({atMs:Date.now()-report.captureStartMs,event:e}));
 ...
 const input=page.getByLabel('What should the agent do?',{exact:true});await input.click();await input.pressSequentially(report.prompt,{delay:23});mark('typing_end');
 await delay(1600);
 mark('start_click');report.taskStartMs=Date.now();
 await page.getByRole('button',{name:'Start task',exact:true}).click();
 await wait(()=>page.evaluate(async()=>['completed','failed','stopped','handoff'].includes((await window.workbench.getRun()).run.state)),180000);
 ...
 await delay(7000);mark('capture_end');
...
}catch(e){console.error(e);fs.writeFileSync(dir+'/evidence/capture-error.txt',String(e));process.exitCode=1;}
