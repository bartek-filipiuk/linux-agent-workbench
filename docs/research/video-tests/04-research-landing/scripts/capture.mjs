...
if(fs.existsSync(dir+'/public/capture.mp4'))throw Error('Preserve the existing capture and evidence before making another take.');
...
let recorder;
const report={captureDate:new Date().toISOString(),prompt:fs.readFileSync(dir+'/evidence/task.txt','utf8'),events:[],markers:[]};
...
try{
 ...
 await page.getByLabel('Browser engine',{exact:true}).selectOption('jev-auto');
 console.log('Ready UI');
 
 await page.screenshot({path:dir+'/evidence/ready.png'});
 ...
 await page.exposeFunction('recordEvent',async e=>{
  ...
  fs.appendFileSync(dir+'/evidence/events-live.jsonl',JSON.stringify(item)+'\n',{mode:0o600});
 ...
 const log=fs.openSync(dir+'/evidence/ffmpeg.log','w');
 recorder=spawn('ffmpeg',['-hide_banner','-y','-f','x11grab','-framerate','30','-video_size','1600x900','-draw_mouse','0','-i',display+'.0','-an','-c:v','libx264','-preset','ultrafast','-crf','16','-pix_fmt','yuv420p',dir+'/public/capture.mp4'],{stdio:['pipe','ignore',log]});
 ...
 const input=page.getByLabel('What should the agent do?',{exact:true});await input.click();await input.fill(report.prompt);mark('typing_end');
 await delay(4200);
 mark('start_click');report.taskStartMs=Date.now();
 await page.getByRole('button',{name:'Start task',exact:true}).click();
 await wait(()=>page.evaluate(async()=>['completed','failed','stopped','handoff','budget_paused','budget_exceeded'].includes((await window.workbench.getRun()).run.state)),1200000);
 ...
 await page.screenshot({path:dir+'/evidence/result.png'});
 ...
 fs.writeFileSync(dir+'/evidence/run.json',JSON.stringify(report,null,2),{mode:0o600});
