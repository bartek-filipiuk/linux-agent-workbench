...
const instance=process.env.LAW_CAPTURE_INSTANCE ?? 'tutorial-research';
...
try{
 ...
 await delay(1000);
 let switched=false;
 await page.exposeFunction('recordEvent',async e=>{report.events.push({atMs:Date.now()-report.captureStartMs,event:e});if(!switched&&e.type==='run.tool'&&e.status==='executing'&&/terminal|exec_command/.test(e.name)){switched=true;report.markers.push({name:'view_terminal',atMs:Date.now()-report.captureStartMs});await page.getByRole('tab',{name:'Terminal',exact:true}).click();}});
 ...
 const input=page.getByLabel('What should the agent do?',{exact:true});await input.click();await input.pressSequentially(report.prompt,{delay:11});mark('typing_end');
 await delay(1600);
 mark('start_click');report.taskStartMs=Date.now();
 await page.getByRole('button',{name:'Start task',exact:true}).click();
 await wait(()=>page.evaluate(async()=>['completed','failed','stopped','handoff'].includes((await window.workbench.getRun()).run.state)),240000);
...
}catch(e){console.error(e);fs.writeFileSync(dir+'/evidence/capture-error.txt',String(e));fs.writeFileSync(dir+'/evidence/run-error.json',JSON.stringify(report,null,2),{mode:0o600});process.exitCode=1;}
