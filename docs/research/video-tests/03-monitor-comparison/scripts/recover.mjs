...
if(fs.existsSync(dir+'/public/recovery.mp4'))throw Error('Preserve the existing capture and evidence before making another take.');
...
let recorder;
const report={captureDate:new Date().toISOString(),prompt:fs.readFileSync(dir+'/evidence/recovery-task.txt','utf8'),events:[],markers:[]};
...
try{
 ...
 await page.screenshot({path:dir+'/evidence/recovery-ready.png'});
 ...
 await page.exposeFunction('recordEvent',async e=>{
  ...
  fs.appendFileSync(dir+'/evidence/recovery-events-live.jsonl',JSON.stringify(item)+'\n',{mode:0o600});
 ...
 const log=fs.openSync(dir+'/evidence/recovery-ffmpeg.log','w');
 recorder=spawn('ffmpeg',['-hide_banner','-y','-f','x11grab','-framerate','30','-video_size','1600x900','-draw_mouse','0','-i',display+'.0','-an','-c:v','libx264','-preset','ultrafast','-crf','16','-pix_fmt','yuv420p',dir+'/public/recovery.mp4'],{stdio:['pipe','ignore',log]});
 ...
 await page.screenshot({path:dir+'/evidence/recovery-result.png'});
 ...
 fs.writeFileSync(dir+'/evidence/recovery.json',JSON.stringify(report,null,2),{mode:0o600});
