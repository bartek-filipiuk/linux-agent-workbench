...
const api = {
  ...
  sendFollowup: (runId: string, message: string) => ipcRenderer.invoke("conversation:send", runId, message),
  pauseRun: (runId: string) => ipcRenderer.invoke("conversation:pause", runId),
  ...
  startRun: (goal: string, opts?: { browserEngine?: "classic" | "jev-hybrid" | "jev-first" | "jev-auto"; profile?: "quick" | "research" | "project"; maxTurns?: number; modelSelection?: ModelSelection; limits?: RunLimits; playbook?: string }) => ipcRenderer.invoke("run:start", goal, opts),
  listPlaybooks: () => ipcRenderer.invoke("playbooks:list"),
  acceptPlaybook: (slug: string) => ipcRenderer.invoke("playbooks:accept", slug),
  discardPlaybook: (slug: string) => ipcRenderer.invoke("playbooks:discard", slug),
  openPlaybook: (slug: string, draft: boolean) => ipcRenderer.invoke("playbooks:open", slug, draft),
  onPlaybookDraft: on<{ slug: string; name: string }>("playbooks:draft"),
