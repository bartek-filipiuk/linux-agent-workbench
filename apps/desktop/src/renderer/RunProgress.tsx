...
export const RunProgress = memo(function RunProgress({ run }: { run: RunView }) {
  ...
  return <div className="run-progress">
    ...
    {run.browserEngine && <p className="composer-help">Browser: {run.browserEngine === "jev-auto" ? "Jev Auto" : run.browserEngine === "jev-first" ? "Jev First" : run.browserEngine === "jev-hybrid" ? "Jev Hybrid" : "Classic"}</p>}
