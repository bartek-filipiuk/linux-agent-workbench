...
export type TaskPreferences = { browserEngine: "classic" | "jev-hybrid" | "jev-first" | "jev-auto"; profile: keyof typeof STYLES; stepMode: "unlimited" | "custom"; steps: string; timeMode: "none" | "30" | "60" | "custom"; minutes: string; playbook: string };
export const DEFAULT_TASK_PREFERENCES: TaskPreferences = { browserEngine: "classic", profile: "quick", stepMode: "unlimited", steps: "100", timeMode: "30", minutes: "30", playbook: "" };
...
export function readTaskPreferences(raw: string | null): TaskPreferences {
  try {
    ...
    return { browserEngine: (p.browserEngine === "jev-hybrid" || p.browserEngine === "jev-first" || p.browserEngine === "jev-auto") ? p.browserEngine : "classic", profile: p.profile, stepMode: p.stepMode, steps: p.steps, timeMode: p.timeMode, minutes: p.minutes, playbook: typeof p.playbook === "string" && /^[a-z0-9][a-z0-9-]{0,63}$/.test(p.playbook) ? p.playbook : "" };
