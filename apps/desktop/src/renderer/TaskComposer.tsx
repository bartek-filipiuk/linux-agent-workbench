...
export const TaskComposer = memo(function TaskComposer({ workspace, ready, expanded, onExpand, continuation, onStarted }: {
  ...
  return <section className={`task-composer${expanded ? " expanded" : ""}`} aria-label="New task settings">
    ...
    <p className="composer-help" id="browser-engine-help">{preferences.browserEngine === "classic" ? "The selected model controls the browser and terminal." : preferences.browserEngine === "jev-auto" ? "Your selected model chooses how to handle each part of the task. Jev may handle browser steps; page text is sent to TypeSafe. Jev API usage is billed separately, with a $10 task limit." : "Jev handles browser steps; your selected model plans, checks the result and handles exceptions. Page text is sent to TypeSafe. Jev API usage is billed separately, with a $10 task limit."}</p>
